...
def read_image(filename):
	...
	img = None
	file = open(filename)
	if file.readline()[:2] == "P6":
		width, height = file.readline().split()[:2]
		width = int(width)
		height = int(height)
		# print("height = ", height, "width = ", width)
		img = [[[0 for _ in range(3)] for _ in range(width)] for _ in range(height)]
		peakvalue = int(file.readline().split()[0])
		# print(peakvalue)
		# lineno = 0
		# for line in file:
		# 	print("//////// ", line)
		...
		# 		for j in range(3):
		# 			img[lineno][width][j] = .read(8)
		# 	lineno+=1

		for lineno in range(height):
			for i in range(width):
				for j in range(3):
					img[lineno][i][j] = int("{:02x}".format(ord(file.read(1))),16)
						# print(lineno)
	else:
		file = open(filename)
		if file.readline()[:2] =="P5":
			# print("Reached greyscale")
			width, height = file.readline().split()[:2]
			width = int(width)
			height = int(height)
			# print("height = ", height, "width = ", width)
			...
			for lineno in range(height):
				for i in range(width):
					img[lineno][i] = int("{:02x}".format(ord(file.read(1))),16)
	# todo: task5
	...
	for y in img:
		for x in y:
			if isinstance(x, list):
				data.append(tuple([float(i) for i in x]))
			else:
				data.append(float(x))
	# todo: task5
...
def label_image(img, cluster_centroids):
	...
	for i in range(height):
		for j in range(width):
			# print(img[i][j])
			distancelist = map(lambda x: distance(img[i][j],x),cluster_centroids)
			# print(distancelist.index(min(distancelist)))
			cluster_labels[i][j] = distancelist.index(min(distancelist))
	# todo: task6
...
def write_image(filename, img):
	...
	if isinstance(img[0][0], list):
		...
		file.write(str(max(max(max(img)))) + "\n")
		for i in range(height):
			for j in range(width):
				file.write(bytearray(img[i][j]))
	else:
		file.write("P5\n")
		height = len(img)
		width = len(img[0])
		file.write(str(width) + " " + str(height) + "\n")
		file.write(str(max(max(img))) + "\n")
		for i in range(height):
				# str_to_write = hex(img[i][j])[2:]
				# print(str_to_write)
				# if len(str_to_write) == 1:
				# 	str_to_write = "0" + str_to_write
				# string_to_write = str_to_write.decode("hex")
			file.write(bytearray(img[i]))
			# file.write("\n")
