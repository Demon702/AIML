...
class SudokuSearchProblem:
    ...
    def getStartState(self):
        ...
        # self.start_values = parse_grid(self.start_values)
    ...
    def getSuccessors(self, state):
        ...
        for s,d in state.items():
            if len(d) < min_length and len(d)>1:
                min_length = len(d)
                square = s
        # print(square, state[square])
        for action in state[square]:
            # print(action)
            if assign(state.copy(), square,action):
                # display(assign(state.copy(),square,action))
                successors.append([assign(state.copy(),square,action), [square , action], 1])
